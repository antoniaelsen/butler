...
class Sampler:
  ...
  def record(self):
    logger.debug(f'Recording sample...')
    ...
    logger.debug(f" - Recording time: {RECORD_SECONDS} secs")
    ...
    for i in range(0, int(self.sample_rate / FRAME_SIZE * RECORD_SECONDS)):
  ...
  def save(self, filename):
    ...
    output.close()
    logger.info(f'Sample file saved')
